...
<body class="childrenBody">
	...
	<div class="layui-row layui-col-space10 panel_box">
		...
		<div class="panel layui-col-xs12 layui-col-sm6 layui-col-md4 layui-col-lg3">
			<a href="javascript:;">
				...
				<div class="panel_word">
					<span class="loginTime">{{ session('login_at') }}</span>
					<cite>上次登录时间</cite>
				</div>
			</a>
		</div>
	...
	<blockquote class="layui-elem-quote main_btn">
		<p class="layui-red">注意：</p>
		<p>1.题目导入后，记得右上角生成题目</p>
		<p>2.若关卡、题目打开接口异常，请重新打开这个加载页</p>
		<p>3.项目配置文件在根目录下的 .env文件（数据库配置）</p>
        <p>4.日志文件存放在项目文件夹下的 <b>/storage/logs</b> 内</p>
	</blockquote>
        ...
        option = {
            ...
            toolbox: {
                feature: {
                    ...
                    saveAsImage: {'name':"{{ $days[9] }}至{{ $days[0] }}"}
                }
            },
            legend: {
                data:['用户注册','答题次数']
            ...
            xAxis: {
                type: 'category',
                boundaryGap: false,
                data: ["{{ $days[9] }}","{{ $days[8] }}","{{ $days[7] }}","{{ $days[6] }}","{{ $days[5] }}","{{ $days[4] }}","{{ $days[3] }}","{{ $days[2] }}","{{ $days[1] }}","{{ $days[0] }}"]
            },
            yAxis: {
                type: 'value'
            },
